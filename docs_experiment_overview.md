...
*Note on Keyboard Transfer:* The keyboard encoder is pretrained via biometric subject identification rather than a stress-labeled task; its stress-relevance is a plausible but unvalidated transfer hypothesis.

*Note on Handwriting Transfer:* The available handwriting dataset was evaluated through a Neuroticism/personality proxy task rather than a stress-labeled task. The resulting encoder did not demonstrate useful predictive performance and is therefore not transferred into the stress fusion model. The Handwriting branch remains architecturally present but untrained pending a genuinely stress-relevant handwriting dataset.
...
## WESAD Dataset
WESAD (Wearable Stress and Affect Detection Dataset) is included in this repository strictly as a physiological reference/base dataset. It is not one of the five non-contact runtime modalities. It may be used for physiological reference, auxiliary research, comparison, or future experiments where explicitly documented.

## Handwriting & Personality Traits Dataset
This dataset is used as the development dataset for the Handwriting behavioral modality (Modality E). It supports extracting features such as stroke width, speed approximations, and slant. Features that cannot be genuinely obtained from static images (like dynamic pen-up duration) are omitted or marked unavailable in the pipeline.
